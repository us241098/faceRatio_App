...
public class FaceGraphic extends GraphicOverlay.Graphic {
    ...
    public void draw(Canvas canvas) {
       ...
       // List<FirebaseVisionPoint> fface =
         //       face.getContour(FirebaseVisionFaceContour.FACE).getLandmarks();

        //for (int i = 0; i < fface.size(); ++i) {
          //  drawLandmarkPosition(canvas, face, fface.get(i));
        //}
        ...
        double eyeMidX = (leftEyePos.getX() + rightEyePos.getX())/2.0;
        double eyeMidY = (leftEyePos.getY() + rightEyePos.getY())/2.0;
        double hDistance = sqrt(pow((leftEyePos.getX() - rightEyePos.getX()),2) + pow((leftEyePos.getY() - rightEyePos.getY()),2));
        ...
        double mouthMidX = (leftMouthPos.getX() + rightMouthPos.getX())/2.0;
        double mouthMidY = (leftMouthPos.getY() + rightMouthPos.getY())/2.0;

        FirebaseVisionFaceLandmark mBottom = face.getLandmark(FirebaseVisionFaceLandmark.MOUTH_BOTTOM);
        FirebaseVisionPoint mBottomPos = mBottom.getPosition();
        double vDistance = sqrt(pow((eyeMidX-mBottomPos.getX()),2) + pow((eyeMidY-mBottomPos.getY()),2));

        double ratio= hDistance/vDistance;
        ...
        if (facing == CameraSource.CAMERA_FACING_FRONT) {
            ...
            canvas.drawText(
                    "horizontal & vertical dist  : " + String.format("%.2f  ", hDistance)+ String.format(" %.2f", vDistance),
                    x - ID_X_OFFSET,
                    y,
                    idPaint);
            canvas.drawText(
